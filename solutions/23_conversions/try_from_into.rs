...
// différence est que cela doit renvoyer un type `Result` au lieu du type cible
// lui-même. Tu peux en savoir plus dans la documentation :
...
#[derive(Debug, PartialEq)]
enum IntoColorError {
    // Longueur incorrecte de la slice
    BadLen,
...
impl TryFrom<[i16; 3]> for Color {
    ...
    fn try_from(arr: [i16; 3]) -> Result<Self, Self::Error> {
        // Réutilise l'implémentation pour un tuple.
...
fn main() {
    ...
    let v = vec![183, 65, 14];
    // Avec les slices, nous devrions utiliser la fonction `try_from`
    let c3 = Color::try_from(&v[..]);
    println!("{c3:?}");
    // ou mettre la slice entre parenthèses et utiliser `try_into`.
